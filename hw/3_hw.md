...
3. Реализовать мапперы из DTO в клиентские модели данных.
...
#### UI
1. Если в течении загрузки сервер выкинул ошибку (например в кейсе, если сервер не включён и запрос не прошёл), то показываем пользователю, что проблема на стороне сервера. Пример: ![loader](../error_from_be.jpg) //todo обновить картинку
